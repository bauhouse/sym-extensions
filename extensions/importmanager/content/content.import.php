<?php
	...
	class contentExtensionImportManagerImport extends AdministrationPage {
	...
	-------------------------------------------------------------------------*/
		...
		public function __viewIndex() {
			...
			$this->addScriptToHead(URL . '/extensions/importmanager/assets/jquery.js');
			$this->addScriptToHead(URL . '/extensions/importmanager/assets/form.js');
			$this->addStylesheetToHead(URL . '/extensions/importmanager/assets/form.css', 'screen');
		...
		// Importer -----------------------------------------------------------
			...
			$select->setAttribute('id', 'importmanager-importer');
		...
		// External -----------------------------------------------------------
			...
			$input->setAttribute('id', 'importmanager-source');
			...
			if (!empty($this->_status)) {
				...
				// Invalid entry:
					...
					// Source -------------------------------------------------
						...
						$source = htmlentities($xml->saveXML(), ENT_COMPAT, 'UTF-8');
				...
				// Passed:
					...
					$fieldset->appendChild(new XMLElement(
						'p', sprintf(
							'Import completed successfully, %d entries where created.', count($entries)
						)
					));
				}
